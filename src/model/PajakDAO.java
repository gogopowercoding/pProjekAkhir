...
public class PajakDAO {
    public DefaultTableModel getAllPajak() {
        ...
        try (Connection conn = KoneksidB.getKoneksi()) {
            ResultSet rs = conn.createStatement().executeQuery("SELECT * FROM pajak JOIN kendaraan ON pajak.id_kendaraan = kendaraan.id");
            while (rs.next()) {
    ...
    public DefaultTableModel getPajakByStatus(String status) {
        ...
        DefaultTableModel model = new DefaultTableModel(columns, 0);
        try (Connection conn = KoneksidB.getKoneksi()) {
            PreparedStatement stmt = conn.prepareStatement("SELECT * FROM pajak JOIN kendaraan ON pajak.id_kendaraan = kendaraan.id WHERE status = ?");
